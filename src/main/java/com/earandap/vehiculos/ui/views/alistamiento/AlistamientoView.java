 ...
 */
@SpringView(name = AlistamientoView.VIEW_NAME)
public class AlistamientoView extends SecureView implements EditDeleteButtonValueRenderer.EditDeleteButtonClickListener {

    public static final String VIEW_NAME = "alistamiento";

    @Autowired
    private EventBus.ApplicationEventBus applicationEventBus;

    @Autowired
    private AlistamientoFormWindow window;

    @Autowired
    private AlistamientoGrid grid;
    ...
    public void init() {
        ...
        bar.addItem("", FontAwesome.PLUS_SQUARE, menuItem -> {
            AlistamientoFormWindow win = (AlistamientoFormWindow) window.show(-1);
            UI.getCurrent().addWindow(win);
        });
        content = new PortletView("Listado de Alistamiento", FontAwesome.ADN, bar);
    ...
    public void enter(ViewChangeListener.ViewChangeEvent viewChangeEvent) {
        ...
        grid.setData(accionAlistamientoRepository.findAll());
    }
    ...
    public void onEdit(ClickableRenderer.RendererClickEvent event) {
        ...
        AlistamientoFormWindow win = (AlistamientoFormWindow) window.show(id);
        UI.getCurrent().addWindow(win);
    }
        ...
        messageBox.getButton(ButtonId.YES).addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent e) {
                try {
                    ...
                    accionAlistamientoRepository.delete(elemId);
                    if (accionAlistamientoRepository.findOne(elemId) != null) {
                        StaticMembers.showNotificationWarning("Advertencia", "El elemento no pudo ser eliminado porque posee dependencias");
                    } else {
                        applicationEventBus.publish(AlistamientoGrid.ELIMINADO_ALISTAMIENTO, this, event.getItemId());
