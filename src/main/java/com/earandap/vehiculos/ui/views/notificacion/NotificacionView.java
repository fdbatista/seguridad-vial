...
import com.earandap.vehiculos.ui.views.persona.PersonaGrid;
 ...
 */
@SpringView(name = NotificacionView.VIEW_NAME)
public class NotificacionView extends SecureView implements EditDeleteButtonValueRenderer.EditDeleteButtonClickListener {

    public static final String VIEW_NAME = "notificaciones";
    
    @Autowired
    private EventBus.ApplicationEventBus applicationEventBus;

    @Autowired
    private NotificacionFormWindows window;

    @Autowired
    private NotificacionGrid grid;
    ...
    public void init() {
        ...
        bar.addItem("", FontAwesome.PLUS_SQUARE, menuItem -> {
            NotificacionFormWindows win = (NotificacionFormWindows) window.show(-1);
            UI.getCurrent().addWindow(win);
        });
        content = new PortletView("Listado de Notificaciones", FontAwesome.ADN, bar);
    ...
    public void enter(ViewChangeListener.ViewChangeEvent viewChangeEvent) {
        ...
        grid.setData(notificacionRepository.findAll());
    }

    @Override
    public void onEdit(ClickableRenderer.RendererClickEvent event) {
        long id = (long) event.getItemId();
        NotificacionFormWindows win = (NotificacionFormWindows) window.show(id);
        UI.getCurrent().addWindow(win);
    }
        ...
        messageBox.getButton(ButtonId.YES).addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent e) {
                try {
                    ...
                    notificacionRepository.delete(elemId);
                    if (notificacionRepository.findOne(elemId) != null) {
                        StaticMembers.showNotificationWarning("Advertencia", "El elemento no pudo ser eliminado porque posee dependencias");
                    } else {
                        applicationEventBus.publish(NotificacionGrid.ELIMINADO_NOTIFICACION, this, event.getItemId());
