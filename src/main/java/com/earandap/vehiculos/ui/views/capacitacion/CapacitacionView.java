...
import static com.earandap.vehiculos.components.StaticMembers.rutaFicherosEmpresas;
 ...
 */
@SpringView(name = CapacitacionView.VIEW_NAME)
public class CapacitacionView extends SecureView implements EditDeleteButtonValueRenderer.EditDeleteButtonClickListener {

    @Autowired
    private EventBus.ApplicationEventBus applicationEventBus;

    public static final String VIEW_NAME = "capacitacion";

    @Autowired
    private CapacitacionGrid grid;
    ...
    @Autowired
    private CapacitacionFormWindows window;
    ...
    public void init() {
        ...
        bar.addItem("", FontAwesome.PLUS_SQUARE, menuItem -> {
            CapacitacionFormWindows win = (CapacitacionFormWindows) window.show(-1);
            UI ui = UI.getCurrent();
            ui.addWindow(win);
            // UI.getCurrent().addWindow(win);
        });
        content = new PortletView("Cronograma de Actividades", FontAwesome.CALENDAR, bar);
    ...
    public void enter(ViewChangeListener.ViewChangeEvent viewChangeEvent) {
        ...
        grid.setData(capacitacionRepository.findAll());
    }

    @Override
    public void onEdit(ClickableRenderer.RendererClickEvent event) {
        long id = (long) event.getItemId();
        CapacitacionFormWindows win = (CapacitacionFormWindows) window.show(id);
        UI.getCurrent().addWindow(win);
    }
        ...
        messageBox.getButton(ButtonId.YES).addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent e) {
                try {
                    ...
                    Set<CapacitacionDocumento> documentos = grid.container.getItem(elemId).getBean().getDocumentosCapacitacion();
                    capacitacionRepository.delete(elemId);
                    if (capacitacionRepository.findOne(elemId) != null) {
                        StaticMembers.showNotificationWarning("Advertencia", "El elemento no pudo ser eliminado porque posee dependencias");
                    } else {
                        StaticMembers.eliminarCarpetaActividad(parametrosRepository.findOne(Long.valueOf("2")).getValorParametro(), elemId);
                        applicationEventBus.publish(CapacitacionGrid.ELIMINADO_CAPACITACION, this, event.getItemId());
